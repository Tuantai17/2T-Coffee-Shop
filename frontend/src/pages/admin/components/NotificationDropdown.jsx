...
function NotificationDropdown() {
  ...
  const fetchNotifications = useCallback(async () => {
    try {
      ...
      const [listRes, countRes] = await Promise.all([
        getAdminNotifications({ size: 8, sort: "createdAt,desc" }),
        getAdminUnreadCount()
      ]);
      ...
      setError(true);
      // Fallback for visual testing when API fails
      setNotifications([]);
      setUnreadCount(0);
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (isOpen && !event.target.closest("#admin-notification-dropdown")) {
        setIsOpen(false);
      }
    };
  ...
  return (
    <div className="position-relative" id="admin-notification-dropdown">
      <div 
        className="position-relative cursor-pointer neu-circle shadow-sm" 
        onClick={handleToggle}
        aria-label="Mở thông báo"
        aria-expanded={isOpen}
        style={{ width: "45px", height: "45px", display: "flex", alignItems: "center", justifyContent: "center", backgroundColor: "white", borderRadius: "50%", cursor: "pointer" }}
      >
        <i className="fa-regular fa-bell" style={{ color: "var(--admin-muted)", fontSize: "1.2rem" }}></i>
        {unreadCount > 0 && (
          <span 
            className="position-absolute badge rounded-pill bg-danger border border-2 border-white shadow-sm"
            style={{ top: "0px", right: "-2px", fontSize: "0.6rem", padding: "0.35em 0.5em" }}
          >
