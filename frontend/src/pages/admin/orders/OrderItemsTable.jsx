...
const OrderItemsTable = ({ items = [], onReportIssue, issues = [], onResolveIssue, orderStatus }) => {
  ...
  const calculatedTotal = items.reduce((sum, item) => {
    ...
    const finalQty = item.finalQuantity !== null ? item.finalQuantity : item.quantity;
    const price = item.unitPrice || item.product?.price || 0;
    return sum + (price * finalQty);
  }, 0);
      ...
      <table className="table align-middle mb-0" style={{ fontSize: '13px' }}>
        ...
        <tbody>
          {items.map((item, index) => {
            ...
            const finalQty = item.finalQuantity !== null ? item.finalQuantity : originalQty;
            ...
            return (
              <tr key={item.id} className={rowClass}>
                ...
                <td className="text-end fw-bold text-dark">
                  {isRemoved ? <span className="text-muted">0 ₫</span> : formatCurrency(unitPrice * finalQty)}
                </td>
